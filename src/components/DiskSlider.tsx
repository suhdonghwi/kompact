...
function DiskSlider({ email }: { email: string }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const q = query<DiskDoc>(disksRef, where('email', '==', email));
      const querySnapshot = await getDocs<DiskDoc>(q);
      setItems(
        querySnapshot.docs.map((doc) => ({
          id: doc.id,
          name: doc.name,
          images: [],
          cover: `${doc.id}/cover.png`,
        })),
      );
    })();
  }, []);
...
function AnimatedDisk({ cover }: { cover: string }) {
  ...
  const bind = useGesture(
    {
      onDrag: ({ down, movement: [mx, my], offset: [x, y] }) => {
